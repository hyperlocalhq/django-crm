...
{
   echo ""
   echo "Usage: ./full_restore_aidas.sh -e staging|production"
...
BACKUP_PATH=${CURRENT_DIR}/latest.backup

echo "Downloading ${ENV} database backup..."
date

scp ${ENV}_${PROJECT}:/var/webapps/${PROJECT}/db_backups/latest.backup.gz ${BACKUP_PATH}.gz
...
if [ ! -f ${BACKUP_PATH} ]; then
    echo "Backup file not found! Run ./full_restore_aidas.sh -e 'staging|production' first."
    exit 1
fi
...
python manage.py migrate --settings=${PROJECT}.settings.local_aidas
python manage.py resetsuperuser --settings=${PROJECT}.settings.local_aidas
